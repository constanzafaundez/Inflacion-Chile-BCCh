---
title: "**Automatización del Cálculo de Inflación en Chile: Extracción de Datos del IPC mediante API del Banco Central en R**"
author: "Constanza Faúndez"
output: 
  html_document:
    toc: true             # Tabla de contenido automática
    toc_depth: 2          # Dos niveles de encabezados
    theme: flatly         # Diseño profesional
    highlight: tango      # Sintaxis de código coloreada
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(
  echo = TRUE,           # Muestra el código en el reporte
...
El stack de econometría implementado en R requiere las siguientes librerías clave:

```{r libreries}
library(httr)       # Conexión segura a APIs REST (protocolos HTTPS)
library(jsonlite)   # Procesamiento eficiente de datos JSON (estructura API BCCh)
library(lubridate)  # Manipulación avanzada de fechas (serie temporal mensual)
library(dplyr)      # Transformación de datos con sintaxis tidyverse (pipe operator %>%)
library(ggplot2)    # Visualización estática profesional (gramática de gráficos)
library(plotly)     # Dashboards interactivos (hover tooltips, zoom dinámico)
library(zoo)        # Cálculo de indicadores financieros (rolling averages, lags) 
```
...
knitr::kable(inflacion_actual, caption = "Indicadores de Inflación - Último Mes Disponible")
```

La tabla muestra la situación inflacionaria actual, permitiendo:

- Comparación con metas institucionales 
- Evaluación de tendencias recientes
- Identificación de presiones de precios
...
<center>
📌 *Documento generado con R Markdown v`r packageVersion('rmarkdown')`*  
</center>
